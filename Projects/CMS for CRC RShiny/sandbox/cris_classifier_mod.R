...
#  supprsessWarnings()
  ...
  if (FDR.sample.bar!="NA"){
    FDR.sample.bar <- as.numeric(FDR.sample.bar) #add FDS to sample bar
    if (is.numeric(FDR.sample.bar)==FALSE){ #check variable type
  ...
  if (length(grep("gct$", input.exp.filename))>0){
      exp.dataset<-read.table(input.exp.filename)
  ...
  if (length(grep("gct$", input.exp.filename))>0){
    ...
    exp.for.sample.names<- read.table(input.exp.filename)
  ...
  normed.exp.dataset<-cbind(ProbeID,normed.exp.dataset) #error
