...
#try to plot only the tcga plot using ggplot
tcga.ref_umap.dat_2 <- data.frame("umap1" = tcga.ref_umap$layout[, 1], 
                                "umap2" = tcga.ref_umap$layout[, 2],
                                "umap3" = tcga.ref_umap$layout[, 3])

# Plot 3D scatter plot using ggplot2 with multiple 2D plots

#plot for umap1 and umap2
plot1 <- ggplot(tcga.ref_umap.dat_2, aes(x = umap1, y = umap2)) +
  geom_point() +
  labs(title = "UMAP: Dimension 1 vs Dimension 2")+
  stat_ellipse(data = tcga.ref_umap.dat, 
               aes(x = umap1, y = umap2, color = cms.lab), 
               type = "norm", linetype = 2, level = 0.95)+
  geom_point(data = tcga.ref_umap.dat, 
...
plot1
plot2 <- ggplot(tcga.ref_umap.dat_2, aes(x = umap1, y = umap3)) +
  geom_point() +
  labs(title = "UMAP: Dimension 1 vs Dimension 3")

plot3 <- ggplot(tcga.ref_umap.dat_2, aes(x = umap2, y = umap3)) +
  geom_point() +
  labs(title = "UMAP: Dimension 2 vs Dimension 3")

# Arrange the plots in a grid
multiplot <- cowplot::plot_grid(plot1, plot2, plot3, ncol = 2)

# Print the grid of plots
print(multiplot)

#plot the cohort in ggplot with colors
